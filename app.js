...
function init(){
 ...
 if(input){ const initialQ=new URLSearchParams(location.search).get('q')||''; if(initialQ) input.value=initialQ; input.addEventListener('input',()=>runSearch(input.value.trim())); input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault(); const q=input.value.trim(); gooDaariTrack('search',{search_term:q,location:selectedTownName()}); location.href='/search.html'+(q?'?q='+encodeURIComponent(q):'');}})}
 if(btn)btn.addEventListener('click',()=>{const q=input?.value.trim()||''; gooDaariTrack('search',{search_term:q,location:selectedTownName()}); location.href='/search.html'+(q?'?q='+encodeURIComponent(q):'');});
 document.querySelectorAll('[data-q]').forEach(b=>b.addEventListener('click',()=>{if(input){input.value=b.dataset.q; location.href='/search.html?q='+encodeURIComponent(b.dataset.q)}}));
