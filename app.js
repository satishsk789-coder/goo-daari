...
function init(){
 ...
 if(input){ const initialQ=new URLSearchParams(location.search).get('q')||''; if(initialQ) input.value=initialQ; input.addEventListener('input',()=>runSearch(input.value.trim())); input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault(); const q=input.value.trim(); if(q){ gooDaariTrack && gooDaariTrack('search',{search_term:q,location:selectedTownName()}); location.href='/search.html?q='+encodeURIComponent(q); }}})}
 if(btn)btn.addEventListener('click',()=>{const q=input?.value.trim()||''; if(q){ gooDaariTrack && gooDaariTrack('search',{search_term:q,location:selectedTownName()}); location.href='/search.html?q='+encodeURIComponent(q); }});
